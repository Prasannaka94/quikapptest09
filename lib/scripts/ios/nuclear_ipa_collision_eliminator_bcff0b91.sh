...
# Nuclear IPA Collision Elimination Script for Error ID bcff0b91
...
SCRIPT_NAME="Nuclear IPA Collision Eliminator (bcff0b91)"
ERROR_ID="bcff0b91"
...
if [ $# -lt 3 ]; then
    ...
    echo "📝 Example: $0 Runner.ipa com.insurancegroupmo.insurancegroupmo bcff0b91"
    exit 1
fi
...
log_info "💥 Strategy: Direct IPA file modification for BCFF0B91 elimination"
...
if [ "$TARGET_ERROR_ID" != "bcff0b91" ]; then
    log_error "This script is specifically for error ID bcff0b91, but $TARGET_ERROR_ID was provided"
    exit 1
fi

# Create working directory
WORK_DIR="nuclear_ipa_workspace_bcff0b91_${TIMESTAMP}"
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
