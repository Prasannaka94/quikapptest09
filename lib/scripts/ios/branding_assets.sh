...
EOF
            ...
            if flutter pub run change_app_package_name:main --config "$config_file"; then
                ...
                log_warn "⚠️ change_app_package_name failed, falling back to manual update..."
                
                # Fallback to manual bundle ID update
                apply_manual_bundle_id_update "$pkg_name"
            fi
            ...
            log_warn "⚠️ Flutter not available, using manual bundle ID update..."
            apply_manual_bundle_id_update "$pkg_name"
        fi
...
# Fallback function for manual bundle ID update (if change_app_package_name fails)
apply_manual_bundle_id_update() {
    local pkg_name="$1"
    
    log_info "🔄 Applying manual bundle ID update as fallback..."
